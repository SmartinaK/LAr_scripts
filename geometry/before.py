...
#        detectorDict[fname] = accumulatedX0
    ...
    for plot, title in zip(["x0", "lambda", "depth"], axis_titles):
        ...
        for i, detector in enumerate(detectorDict.keys()):
            ...
            detectorDict[detector][plot].SetLineColor(linecolor)
            detectorDict[detector][plot].SetFillColor(i + 10)
            detectorDict[detector][plot].SetLineWidth(1)
            detectorDict[detector][plot].SetFillStyle(1001)
